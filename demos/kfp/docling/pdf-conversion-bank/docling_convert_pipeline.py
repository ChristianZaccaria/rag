...
@dsl.component(
    ...
    packages_to_install=["llama-stack-client", "fire", "requests", "rapidocr", "onnxruntime"],
)
def register_vector_db(
    ...
    print(
        f"Registered vector DB '{actual_vector_db_id}' (requested: '{vector_db_id}') with embedding model '{embed_model_id}'."
    )
    return actual_vector_db_id


@dsl.component(
    base_image=PYTHON_BASE_IMAGE,
    packages_to_install=["requests", "rapidocr", "onnxruntime"],
)
...
def docling_convert_pipeline(
    base_url: str = "https://raw.githubusercontent.com/akram/rag-hackaton-demo/main/pdf",
    pdf_filenames: str = "redbankfinancial_about.pdf, redbankfinancial_faq.pdf",
    num_workers: int = 1,
    vector_db_id: str = "red_bank_financial_kb",
    service_url: str = "http://ragathon-team-3-ragathon-team-3.apps.llama-rag-pool-b84hp.aws.rh-ods.com",
    ...
    with dsl.ParallelFor(pdf_splits.output) as pdf_split:
        with dsl.If(use_gpu == True):
            convert_task = docling_convert(
                ...
                service_url=service_url,
                vector_db_id=register_task.output,
            )
